...
file_path = "../bnp_fly0003.h5"

with h5py.File(file_path, "r") as f:
    xrf_fits = f["MAPS/XRF_fits"][:]  # Read full dataset

    # Save the extracted array as a NumPy `.npy` file
    np.save("xrf_fits_3d.npy", xrf_fits)

    # Print shape and data type to confirm
    print(f"Extracted 3D array shape: {xrf_fits.shape}")
    # print(f"Data type: {xrf_fits.dtype}")
    # Extract TFY channel
    image0=xrf_fits[22,:,:]


file_path = "../bnp_fly0002.h5"
...
with h5py.File(file_path, "r") as f:
    ...
    # print(f"Data type: {xrf_fits.dtype}")
    # Extract TFY channel
    image1_original=xrf_fits[22,:,:]

def min_max_normalize(image):
    return (image - np.min(image)) / (np.max(image) - np.min(image))

image0_norm = min_max_normalize(image0)
image1_norm = min_max_normalize(image1_original)
image0 = image0_norm

image1 = transform.resize(image1_norm, image0.shape, anti_aliasing=True)
np.max(image0_norm)
